#!/usr/bin/env python3
"""
简化的 GPT-like 模型训练脚本（DDP 教学版）
支持单机/单GPU、单机/多GPU、以及多机多GPU（基于 torch.distributed）.

使用示例（单机单GPU或CPU）:
    python train_ddp.py --epochs 3 --batch_size 16
...
# -----------------------------
def prepare_data():
    """加载 wikitext-103-raw-v1 数据集的训练集，返回非空文本列表"""
    logger.info("加载 WikiText-103 数据集...")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="简化的 GPT-like 模型训练（DDP 教学版）")
    ...
    if is_distributed:
        # 包裹 DDP：device_ids 在 NCCL 后端下需要传入本地 GPU id
        if torch.cuda.is_available():
    ...
    for epoch in range(args.epochs):
        ...
        if (not is_distributed) or rank == 0:
            ...
            # 如果使用 DDP，保存 module 的 state_dict
            state_dict = model_to_train.module.state_dict() if is_distributed else model_to_train.state_dict()
            torch.save(state_dict, f"checkpoints/model_epoch_{epoch+1}.pth")

    # 最终模型保存（仅主进程）
    if (not is_distributed) or rank == 0:
        final_state = model_to_train.module.state_dict() if is_distributed else model_to_train.state_dict()
