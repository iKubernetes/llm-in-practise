...
- 混合精度训练与模型检查点保存
- 增强错误处理和日志
- 禁用tokenizers并行性以避免fork警告

启动命令：
 - 指定使用两个GPU进行模型训练：
  torchrun --nproc_per_node=2 ddp_gpt_bpe_wikitext2.py
...
def main():
    ...
    parser.add_argument("--vocab_size", type=int, default=30000, help="词汇表大小")
    ...
    world_size = max(1, min(world_size, total_gpus))
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (x, y) in enumerate(dataloader):
            ...
            with torch.amp.autocast('cuda'):  # 更新为新 API
                outputs = model(x)
