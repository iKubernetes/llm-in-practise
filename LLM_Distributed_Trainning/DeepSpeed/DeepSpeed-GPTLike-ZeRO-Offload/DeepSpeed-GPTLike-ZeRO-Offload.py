#!/usr/bin/env python3
"""
DeepSpeed Stage-3 GPT-like 训练脚本（简化版，中文注释）
特点：
- ZeRO Stage-3（参数、梯度、优化器状态全部分片）
- 自动读取 ds_config.json 配置
- 支持 FP16
- 删除 --local_rank 参数，兼容 deepspeed launcher
"""
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="GPT-like 模型训练（DeepSpeed Stage-3）")
    parser.add_argument("--epochs", type=int, default=3, help="训练轮数")
    parser.add_argument("--batch_size", type=int, default=16, help="DataLoader 批次大小（可被 ds_config 覆盖）")
    ...
    parser.add_argument("--ds_config", type=str, default="ds_config_stage3.json", help="DeepSpeed 配置文件")
    ...
    # 加载 ds_config.json 中 train_micro_batch_size_per_gpu
    ds_batch = None
    if args.ds_config and os.path.isfile(args.ds_config):
        try:
            ...
            logger.warning(f"无法读取 ds_config.json: {e}")

    dataloader_batch_size = ds_batch if ds_batch is not None else args.batch_size
    if ds_batch is not None:
        logger.info(f"使用 ds_config.json 的 train_micro_batch_size_per_gpu={ds_batch} 作为 DataLoader 批次大小")
    else:
        logger.info(f"使用命令行 --batch_size={args.batch_size}")
    ...
    if is_main_process:
        logger.info(f"使用设备: {device}, rank={world_rank}/{world_size}")
